  ...
  try {
    ...
    if ("class_id" in req.body) {
      ...
      if (oldClassCode) {
        try {
          const resOld = await axios.get(
            `${process.env.VITE_CLASS_SERVICE_URL}/api/${oldClassCode}`
          );
          const oldClassDocId = resOld.data.class._id;

          await axios.put(
            `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/${oldClassDocId}/remove-teacher`
          );
        } catch (error) {
      ...
      if (newClassCode) {
        try {
          await axios.put(
            `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/assign-teacher`,
            {
...
exports.deleteAdvisor = async (req, res) => {
  try {
    ...
    const classServiceUrl = `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/by-teacher/${userId}`;
    try {
      ...
      if (classData && classData._id) {
        await axios.put(
          `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/${classData._id}/remove-teacher`,
          {
...
exports.fullDeleteStudent = async (req, res) => {
  ...
  try {
    try {
      await axios.delete(
        `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/remove-student-if-exists/${studentId}`
      );
    } catch (removeErr) {
      console.warn(
