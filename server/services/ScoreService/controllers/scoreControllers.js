...
exports.getStudentScoresGroupedBySemester = async (req, res) => {
  try {
    ...
    const semesters = await axios.get(
      `${process.env.VITE_EDUCATION_SERVICE_URL}/api/semesters`
    );
    const semesterMap = {};
...
exports.getStudentScoresBySemester = async (req, res) => {
  try {
    ...
    const subjectsRes = await axios.get(
      `${process.env.VITE_EDUCATION_SERVICE_URL}/api/subjects`
    );
    const subjectMap = {};
...
exports.importStudentScores = async (req, res) => {
  ...
  const teachingClassesRes = await axios.get(
    `${process.env.VITE_CLASS_SERVICE_URL}/api/teacher/tdt/${req.user.tdt_id}`
  );
  ...
  const subjectListRes = await axios.get(
    `${process.env.VITE_EDUCATION_SERVICE_URL}/api/departments/${req.user.tdt_id}/subjects`
  );
  const teacherSubjects = subjectListRes.data || [];
  const teacherSubjectCodes = new Set(
    teacherSubjects.map((s) => s.subject_code)
  );
    ...
    .on("end", async () => {
      ...
      try {
        for (const row of records) {
          try {
            ...
            if (!teacherSubjectCodes.has(subject_code)) {
              try {
                const subjectRes = await axios.get(
                  `${process.env.VITE_EDUCATION_SERVICE_URL}/api/subjects/code/${subject_code}`
                );
                const subject = subjectRes.data;
                skippedStudents.push({
                  tdt_id,
                  reason: `Bạn không có quyền nhập điểm cho môn học: ${
            ...
            const semesterRes = await axios.get(
              `${process.env.VITE_EDUCATION_SERVICE_URL}/api/semesters/code/${semester_code}`
            );
        ...
        if (inserted.length === 0) {
          ...
          const allAreWrongClass = allReasons.every(
            (r) => r === "Sinh viên không nằm trong các lớp mà bạn đang dạy"
          );
...
exports.updateScore = async (req, res) => {
  ...
  try {
    ...
    for (const [category, score] of Object.entries(scores)) {
      ...
      if (!updated) {
        const subjectRes = await axios.get(
          `${process.env.VITE_EDUCATION_SERVICE_URL}/api/subjects/${subject_id}`
        );
...
exports.getStudentScoreboardByTeacher = async (req, res) => {
  ...
  try {
    ...
    const subjectTaught = teacherSubjects[0];
    ...
    if (!scoreboard) {
      return res.status(404).json({
        message: "Không tìm thấy bảng điểm của học sinh trong kỳ này",
      });
    }
    ...
    if (!subjectData) {
      return res
        .status(404)
        .json({ message: "Học sinh không học môn này trong kỳ này" });
    }
...
exports.exportClassScoresPdf = async (req, res) => {
  ...
  try {
    ...
    const classRes = await axios.get(
      `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/${classId}/students`
    );
    const students = classRes.data.students;

    const semesterRes = await axios.get(
      `${process.env.VITE_EDUCATION_SERVICE_URL}/api/semesters/${semesterId}`
    );
    const semester = semesterRes.data;
    ...
    for (const student of students) {
      const user = await axios.get(
        `${process.env.VITE_USER_SERVICE_URL}/api/users/${student._id}`
      );
      const st = user.data;
      doc.addPage();
      doc.fontSize(16).text(`BẢNG ĐIỂM HỌC KỲ ${semester.semester_name}`, {
        align: "center",
      });
      ...
      let scoreRes;
      try {
        scoreRes = await axios.get(
          `${process.env.VITE_SCORE_SERVICE_URL}/api/students/${student._id}/scores`,
          {
      ...
      doc
        .fontSize(12)
        .text(
          `Điểm trung bình học kỳ: ${scoreRes.data.gpa.toFixed(2) || "N/A"}`,
          startX
        );
...
exports.exportClassSummaryPdf = async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=class-summary.pdf"
    );
    ...
    doc
      .fontSize(16)
      .text(
        `BẢNG TỔNG KẾT LỚP ${classRes.data.class_id} - HỌC KỲ ${semester.semester_name}`,
        {
          align: "center",
          underline: true,
        }
    ...
    headers.forEach((header, index) => {
      const x =
        startX + columnWidths.slice(0, index).reduce((acc, w) => acc + w, 0);
    ...
    for (let i = 0; i < students.length; i++) {
      ...
      const row = [i + 1, st.tdt_id || "-", st.name || "-", gpa, status];
...
exports.exportSubjectScoresPdf = async (req, res) => {
  ...
  try {
    ...
    if (!classId || !semesterId || !teacherId) {
      return res
        .status(400)
        .json({ message: "Thiếu classId, semesterId hoặc teacherId" });
    }
    ...
    const teacherRes = await axios.get(
      `${process.env.VITE_EDUCATION_SERVICE_URL}/api/departments/${teacherId}/subjects`
    );
    ...
    if (!Array.isArray(subjectList) || subjectList.length === 0) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy môn học mà giáo viên đang dạy" });
    }

    res.setHeader("Content-Type", "application/pdf");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=subject-score-${subjectInfo.subject_code}.pdf`
    );
    ...
    const printTitle = () => {
      doc
        .fontSize(16)
        .text(
          `BẢNG ĐIỂM MÔN ${subjectInfo.subject_name.toUpperCase()} - LỚP ${
            classRes.data.class_id
          }`,
          { align: "center", underline: true }
        );
      doc
        .fontSize(14)
        .text(`HỌC KỲ: ${semester.semester_name}`, { align: "center" });
      doc.moveDown(1);
      startY = doc.y;
    };
...
exports.sendReportCardsToClassParents = async (req, res) => {
  try {
    ...
    if (!Array.isArray(students) || !students.length) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy học sinh trong lớp" });
    }
    ...
    for (const student of students) {
      try {
        ...
        if (
          ...
          !studentDetail.parent_email
        ) {
          results.push({
            student: studentDetail?.name || "Không rõ",
            status: "Thiếu thông tin học sinh hoặc email phụ huynh",
          });
          continue;
        }

        const teacherRes = await axios.get(
          `${process.env.VITE_CLASS_SERVICE_URL}/api/students/${student._id}/advisor`
        );
        const teacher = teacherRes.data;

        const scoresRes = await axios.get(
          `${process.env.VITE_SCORE_SERVICE_URL}/api/students/${student._id}/scores?semester_id=${semesterId}`
        );
        ...
        const tableHtml = scoreData.scores
          .map((s, idx) => {
            return `<tr><td>${idx + 1}</td><td>${s.subject_code}</td><td>${
              s.subject_name
            }</td><td>${s.score_15p ?? "-"}</td><td>${
              s.score_1tiet ?? "-"
            }</td><td>${s.score_giuaky ?? "-"}</td><td>${
              s.score_cuoiky ?? "-"
            }</td><td><strong>${s.score ?? "-"}</strong></td></tr>`;
          })
          .join("");

        const html = `<h3>Xin chào phụ huynh của học sinh ${
          studentDetail.name
        }</h3><p>Đây là bảng điểm học kỳ của con bạn trong học kì ${
          semester.semester_name
        }:</p><table border="1" cellpadding="5" cellspacing="0"><thead><tr><th>STT</th><th>Mã môn</th><th>Môn học</th><th>15 phút</th><th>1 tiết</th><th>Giữa kỳ</th><th>Cuối kỳ</th><th>Điểm trung bình</th></tr></thead><tbody>${tableHtml}</tbody></table><p>Điểm trung bình học kỳ: <strong>${
          scoreData.gpa?.toFixed(2) || "-"
        }</strong></p><p>Xếp loại học lực: <strong>${
          scoreData.status || "-"
        }</strong></p><p>Người gửi: <strong>GVCN ${
          teacher.advisor?.name || "Không rõ"
        }</strong></p>`;
...
const scheduleReportCardSending = async (
  ...
  if (testMode && testDateTime) {
    ...
    console.log(
      "🧪 Chạy ở chế độ kiểm thử với thời gian:",
      endDateVN.toLocaleString()
    );
  } else {
    const endDateUTC = new Date(semester.end_date);
    endDateVN = new Date(endDateUTC.getTime() + 7 * 60 * 60 * 1000);
    endDateVN.setHours(23, 4, 0, 0);
  }
  ...
  setTimeout(async () => {
    try {
      ...
      for (const classItem of classes) {
        ...
        try {
          await axios.post(
            `${process.env.VITE_SCORE_SERVICE_URL}/api/students/send-report-card/${classId}?semester_id=${semesterId}`
          );
          console.log(`Gửi thành công cho lớp ${classId}`);
        } catch (err) {
          if (err.response && err.response.status === 404) {
            console.log(
              `Lớp ${classItem.name || classId} không có dữ liệu để gửi.`
            );
            continue;
          }
          console.error(
            `Lỗi gửi Weil ${classItem.name || classId}: ${err.message}`
          );
          if (err.response) {
            console.error(
...
exports.reviewPromotion = async (req, res) => {
  ...
  try {
    const semesterRes = await axios.get(
      `${process.env.VITE_EDUCATION_SERVICE_URL}/api/semesters/semester?school_year=${school_year}`
    );
    ...
    const classRes = await axios.get(
      `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/${class_id}`
    );
    ...
    for (const studentId of classMembers) {
      ...
      if (scoreboards.length !== 2) {
        ...
        await axios.post(
          `${process.env.VITE_USER_SERVICE_URL}/api/users/${studentId}/repeat`,
          {
      ...
      for (const scoreboard of scoreboards) {
        ...
        if (
          ["Yếu", "Trung bình", "Khá", "Tốt"].indexOf(studentBehavior) >
          ["Yếu", "Trung bình", "Khá", "Tốt"].indexOf(behavior)
        ) {
      ...
      if (isEligible) {
        ...
        result.repeated.push({
          ...
          reason:
            studentBehavior === "Yếu"
              ? "Hạnh kiểm Yếu ở ít nhất một kỳ"
              : "GPA dưới 5.0 ở ít nhất một kỳ",
        });
        await axios.post(
          `${process.env.VITE_USER_SERVICE_URL}/api/users/${studentId}/repeat`,
          {
...
exports.getStatusAndBehavior = async (req, res) => {
  try {
    ...
    if (!user_id || !semester_ids) {
      return res
        .status(400)
        .json({ message: "Thiếu user_id hoặc semester_ids" });
    }
    ...
    if (semesterIdsArray.length === 0) {
      return res
        .status(400)
        .json({ message: "Danh sách semester_ids không hợp lệ" });
    }
    ...
    if (!scores || scores.length === 0) {
      return res
        .status(404)
        .json({ message: "Không tìm thấy điểm cho học sinh này" });
    }
