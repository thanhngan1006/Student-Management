...
exports.getStudentScoresGroupedBySemester = async (req, res) => {
  try {
    ...
    const semesters = await axios.get(
      `${process.env.EDUCATION_SERVICE_URL}/api/semesters`
    );
    const semesterMap = {};
    semesters.data.forEach((sem) => {
      semesterMap[sem._id] = sem.semester_name;
...
exports.getStudentScoresBySemester = async (req, res) => {
  try {
    ...
    const subjectsRes = await axios.get(
      `${process.env.EDUCATION_SERVICE_URL}/api/subjects`
    );
    const subjectMap = {};
    subjectsRes.data.forEach((sub) => {
      subjectMap[sub._id] = {
...
exports.importStudentScores = async (req, res) => {
  ...
  const teachingClassesRes = await axios.get(
    `${process.env.CLASS_SERVICE_URL}/api/teacher/tdt/${req.user.tdt_id}`
  );
  const teachingClasses = teachingClassesRes.data || [];
  ...
  const subjectListRes = await axios.get(
    `${process.env.EDUCATION_SERVICE_URL}/api/departments/${req.user.tdt_id}/subjects`
  );
  const teacherSubjects = subjectListRes.data || [];
        ...
        for (const row of records) {
          try {
            ...
            if (!teacherSubjectCodes.has(subject_code)) {
              try {
                const subjectRes = await axios.get(
                  `${process.env.EDUCATION_SERVICE_URL}/api/subjects/code/${subject_code}`
                );
                const subject = subjectRes.data;
                skippedStudents.push({
                  tdt_id,
            ...
            const semesterRes = await axios.get(
              `${process.env.EDUCATION_SERVICE_URL}/api/semesters/code/${semester_code}`
            );
            const semester = semesterRes.data;
...
exports.updateScore = async (req, res) => {
  ...
  try {
    ...
    for (const [category, score] of Object.entries(scores)) {
      ...
      if (!updated) {
        const subjectRes = await axios.get(
          `${process.env.EDUCATION_SERVICE_URL}/api/subjects/${subject_id}`
        );
        const subjectCode = subjectRes.data.subject_code;
...
exports.exportClassScoresPdf = async (req, res) => {
  ...
  try {
    ...
    const classRes = await axios.get(
      `${process.env.CLASS_SERVICE_URL}/api/classes/${classId}/students`
    );
    const students = classRes.data.students;

    const semesterRes = await axios.get(
      `${process.env.EDUCATION_SERVICE_URL}/api/semesters/${semesterId}`
    );
    const semester = semesterRes.data;
    ...
    for (const student of students) {
      const user = await axios.get(
        `${process.env.USER_SERVICE_URL}/api/users/${student._id}`
      );
      const st = user.data;
      doc.addPage();
      doc
        .fontSize(16)
        .text(`BẢNG ĐIỂM HỌC KỲ ${semester.semester_name}`, {
          align: "center",
        });
      doc.fontSize(14).text(`Họ tên: ${st.name}`, { align: "left" });
      doc.fontSize(12).text(`Mã học sinh: ${st.tdt_id}`);
      ...
      try {
        scoreRes = await axios.get(
          `${process.env.SCORE_SERVICE_URL}/api/students/${student._id}/scores`,
          {
            params: { semester_id: semesterId },
          }
        );
...
exports.exportSubjectScoresPdf = async (req, res) => {
  ...
  try {
    ...
    const teacherRes = await axios.get(
      `${process.env.EDUCATION_SERVICE_URL}/api/departments/${teacherId}/subjects`
    );
    const subjectList = teacherRes.data;
  ...
  try {
    ...
    for (const student of students) {
      try {
        ...
        const teacherRes = await axios.get(
          `${process.env.CLASS_SERVICE_URL}/api/students/${student._id}/advisor`
        );
        const teacher = teacherRes.data;

        const scoresRes = await axios.get(
          `${process.env.SCORE_SERVICE_URL}/api/students/${student._id}/scores?semester_id=${semesterId}`
        );
        const scoreData = scoresRes.data;
...
const getCurrentSemester = async () => {
  const res = await axios.get(
    `${process.env.EDUCATION_SERVICE_URL}/api/semesters/current`
  );
  return res.data;
};
  ...
  setTimeout(async () => {
    try {
      ...
      const classListRes = await axios.get(
        `${process.env.CLASS_SERVICE_URL}/api/classes/khoi`
      );
      const classes = classListRes.data;
      const semesterId = semester._id;
      ...
      for (const classItem of classes) {
        ...
        try {
          await axios.post(
            `${process.env.SCORE_SERVICE_URL}/api/students/send-report-card/${classId}?semester_id=${semesterId}`
          );
          console.log(`Gửi thành công cho lớp ${classId}`);
        } catch (err) {
          if (err.response && err.response.status === 404) {
...
exports.reviewPromotion = async (req, res) => {
  ...
  try {
    const semesterRes = await axios.get(
      `${process.env.EDUCATION_SERVICE_URL}/api/semesters/semester?school_year=${school_year}`
    );
    const semesters = semesterRes.data;
    ...
    const classRes = await axios.get(
      `${process.env.CLASS_SERVICE_URL}/api/classes/${class_id}`
    );
    const classData = classRes.data;
    ...
    for (const studentId of classMembers) {
      ...
      if (scoreboards.length !== 2) {
        ...
        await axios.post(
          `${process.env.USER_SERVICE_URL}/api/users/${studentId}/repeat`,
          {
            grade: classIdStr.slice(0, 2),
            school_year,
          }
