...
//         "http://localhost:4001/api/semesters/current"
//       );
...
//       console.log(
//         "Sử dụng dữ liệu học kỳ dự phòng (mock):",
    ...
    setTimeout(async () => {
      try {
        ...
        for (const classItem of classes) {
          ...
          try {
            const url = `${process.env.SCORE_SERVICE_URL}/api/students/send-report-card/${classId}?semester_id=${semesterId}`;
            console.log(`Gửi yêu cầu tới: ${url}`);
            await axios.post(url);
            console.log(`Gửi thành công cho lớp ${classId}`);
          } catch (err) {
