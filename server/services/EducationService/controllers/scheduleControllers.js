...
//       "http://localhost:4000/api/classes/khoi"
//     );
...
exports.generateSchedule = async (req, res) => {
  try {
    ...
    if (teacherIds.length > 0) {
      const userResponse = await axios.post(
        `${process.env.USER_SERVICE_URL}/api/users/teachers`,
        { ids: teacherIds }
      );
      users = userResponse.data.map((user) => ({
        user_id: user._id,
    ...
    for (const cls of classes) {
      for (const teacherId of cls.subject_teacher) {
        try {
          ...
          // nên tôi sẽ dùng biến EDUCATION_SERVICE_URL nếu bạn định nghĩa, hoặc gọi trực tiếp.
          // Để an toàn nhất khi deploy, tôi sẽ giả định bạn đã định nghĩa EDUCATION_SERVICE_URL
          const subjectResponse = await axios.get(
            `${
              process.env.EDUCATION_SERVICE_URL || "http://localhost:4001"
    ...
    const result = await axios.post(
      `${process.env.PYTHON_SCHEDULER_URL}/generate-schedule`,
      inputData
    );
