...
exports.createPost = async (req, res) => {
  try {
    ...
    let classRes;
    if (role === "student") {
      classRes = await axios.get(
        `${process.env.VITE_CLASS_SERVICE_URL}/api/students/${author_id}/class`
      );
    } else if (role === "advisor") {
      classRes = await axios.get(
        `${process.env.VITE_CLASS_SERVICE_URL}/api/teachers/${author_id}/class`
      );
    } else {
      return res
        .status(403)
        .json({ message: "Vai trò không hợp lệ để đăng bài" });
    }
    ...
    if (!class_id) {
      return res
        .status(400)
        .json({ message: "Không tìm thấy lớp học của người dùng" });
    }
    ...
    const studentRes = await axios.get(
      `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/${class_id}/students`
    );
    ...
    const advisorRes = await axios.get(
      `${process.env.VITE_CLASS_SERVICE_URL}/api/classes/${class_id}/advisor`
    );
...
exports.toggleLike = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ liked: !alreadyLiked, totalLikes: post.liked_by.length });
  } catch (err) {
...
exports.deletePost = async (req, res) => {
  try {
    const { postId } = req.params;
    const userId = req.user.id; // Usually from token middleware
