...
const Schedule = () => {
  ...
  const [versions, setVersions] = useState<
    { version: number; isApproved: boolean }[]
  >([]);
  ...
  const fetchVersions = async () => {
    ...
    try {
      ...
      setSelectedVersion(
        fetchedVersions.length > 0 ? fetchedVersions[0] : null
      ); // Chọn version cao nhất mặc định
    } catch (err) {
  ...
  const handleApproveVersion = async (version: number) => {
    ...
    try {
      ...
      alert(
        "Lỗi khi duyệt version: " + (err.response?.data?.message || err.message)
      );
    }
  };
  ...
  const scheduleDataByClass = schedules.map((currentSchedule: any) => {
    ...
    ["Day 1", "Day 2", "Day 3", "Day 4", "Day 5", "Day 6"].forEach((day) => {
      const daySchedule = currentSchedule.schedule.find(
        (d: any) => d.day === day
      );
      scheduleData.push({
        day: dayMapping[day],
                  ...
                  {[1, 2, 3, 4, 5].map((period) => (
                    <tr key={period}>
                      ...
                      {[
                        ...
                        const daySchedule = classSchedule.scheduleData.find(
                          (s: any) => s.day === day
                        );
