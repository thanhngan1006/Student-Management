...
const StudentScoresList = () => {
  ...
  useEffect(() => {
    ...
    const fetchSemesters = async () => {
      try {
        ...
        console.error("Lỗi khi lấy danh sách kỳ học:", (err as Error).message);
      }
    };
  ...
  const fetchStudentsByClassId = async (classId: string) => {
    try {
      ...
      console.error("Lỗi khi lấy học sinh lớp:", (err as Error).message);
    }
  };
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      console.error("Lỗi khi xuất PDF:", (err as Error).message);
      alert("Không thể xuất bảng điểm. Vui lòng thử lại!");
    } finally {
      setExportLoading(false);
    }
  ...
  const handleExportSummaryPdf = async () => {
    ...
    try {
      ...
      console.error("Lỗi khi xuất tổng kết PDF:", (err as Error).message);
      alert("Không thể xuất bảng tổng kết. Vui lòng thử lại!");
    } finally {
      setExportSummaryLoading(false);
    }
  ...
  const filterStudents = (term: string, status: string) => {
    const result = originalStudents.filter((s: any) => {
      const matchesSearch =
        s.name.toLowerCase().includes(term) ||
        s.tdt_id?.toLowerCase().includes(term) ||
        s.class_id?.toLowerCase().includes(term);
