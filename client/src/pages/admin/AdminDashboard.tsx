...
interface PromotionResult {
  class_id: string;
  status: string;
  promoted: PromotionStudent[];
  repeated: PromotionStudent[];
  error?: string;
}
...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPendingApprovals = async () => {
      ...
      try {
        ...
        const errorMessage =
          (err as Error)?.message || "Lỗi khi lấy danh sách chờ duyệt.";
        setError(errorMessage);
        console.error("Lỗi khi lấy danh sách chờ duyệt:", errorMessage);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleApproveSubmission = async (approvalId: string) => {
    setLoading(true);
    setError("");
    try {
      if (!token) {
      ...
      setPromotionResults(response.data.promotionResults);
      const status =
        response.data.promotionResults[0]?.status === "success"
          ? "Thành công"
          : "Thất bại";
      setSnackbar({
        open: true,
        message: `Đã duyệt và chuyển lớp thành công: ${status}`,
      });
    } catch (err) {
      const errorMessage =
        (err as Error)?.message ||
        "Lỗi khi duyệt và chuyển lớp. Vui lòng thử lại.";
      setError(errorMessage);
      console.error("Lỗi khi duyệt yêu cầu:", errorMessage);
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    const csv = [
      "Class,Status,Promoted,Repeated,StudentID,Reason",
      ...promotionResults.flatMap((r: PromotionResult) => [
        ...r.promoted.map(
          (p) =>
            `${r.class_id},${r.status},${r.promoted.length},${r.repeated.length},${p.student_id},Promoted`
        ),
        ...r.repeated.map(
          (rp) =>
            `${r.class_id},${r.status},${r.promoted.length},${
              r.repeated.length
            },${rp.student_id},${rp.reason || "N/A"}`
        ),
      ]),
    ].join("\n");
                              ...
                              <tbody>
                                {approval.students.map((student) => (
                                  <tr key={student._id} className="border-t">
                                    ...
                                    <td className="py-2 px-4">
                                      {student.hk1_behavior}
                                    </td>
                                    <td className="py-2 px-4">
                                      {student.hk2_gpa}
                                    </td>
                                    <td className="py-2 px-4">
                                      {student.hk2_behavior}
                                    </td>
                                  </tr>
                                ))}
                              </tbody>
